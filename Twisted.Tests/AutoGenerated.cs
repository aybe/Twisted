...
public static class AutoGenerated
{
    public static void Test(TestContext context, string path)
    {
        ...
        treeNode.TraverseDfsPreOrder(s =>
        {
            switch (s)
            {
        ...
        treeNode.TraverseDfsPreOrder(node =>
        {
            count += 1;
        ...
        treeNode.TraverseDfsPreOrder(s =>
        {
            switch (s)
            {
                case DMDNode0010:
                    Assert.IsTrue(
                        s.Parent is DMDNode020X or DMDNode0B06,
                        $"{s.GetType()}, Parent: {s.Parent.GetType()}"
                    );
                    break;
                case DMDNode00FF:
                    Assert.IsTrue(
                        s.Parent is DMDNode0010 or DMDNode0107 or DMDNode0305 or DMDNode0903 or DMDNode0B06 or DMDNodeXXXX,
                        $"{s.GetType()}, Parent: {s.Parent.GetType()}"
                    );
                    break;
                case DMDNode0107:
                    Assert.IsTrue(
                        s.Parent is DMD or DMDNode0107 or DMDNode040B or DMDNode050B or DMDNode0903 or DMDNode0B06 or DMDNodeXXXX,
                        $"{s.GetType()}, Parent: {s.Parent.GetType()}"
                    );
                    break;
                case DMDNode020X:
                    Assert.IsTrue(
                        s.Parent is DMDNode0107 or DMDNode040B or DMDNode050B,
                        $"{s.GetType()}, Parent: {s.Parent.GetType()}"
                    );
                    break;
                case DMDNode0305:
                    Assert.IsTrue(
                        s.Parent is DMDNode0107,
                        $"{s.GetType()}, Parent: {s.Parent.GetType()}"
                    );
                    break;
                case DMDNode040B:
                    Assert.IsTrue(
                        s.Parent is DMDNode0107 or DMDNode040B or DMDNode050B or DMDNode0903 or DMDNodeXXXX,
                        $"{s.GetType()}, Parent: {s.Parent.GetType()}"
                    );
                    break;
                case DMDNode050B:
                    Assert.IsTrue(
                        s.Parent is DMDNode0107 or DMDNode040B or DMDNode0903 or DMDNodeXXXX,
                        $"{s.GetType()}, Parent: {s.Parent.GetType()}"
                    );
                    break;
                case DMDNode07FF:
                    Assert.IsTrue(
                        s.Parent is DMDNode0107 or DMDNode040B,
                        $"{s.GetType()}, Parent: {s.Parent.GetType()}"
                    );
                    break;
                case DMDNode08FF:
                ...
                case DMDNode0903:
                    Assert.IsTrue(
                        s.Parent is DMDNode0107 or DMDNodeXXXX or DMDNode040B,
                        $"{s.GetType()}, Parent: {s.Parent.GetType()}"
                    );
                    break;
                case DMDNode0B06:
        ...
        // test that all but last ignored regions are before first node, i.e. vertices/normals
        ...
        if (treeNode is DMDNode dmdNode)
        {
            ...
            {
                Assert.IsTrue(region.Position < ((DMDNode)dmdNode[0]).Position, region.ToString());
            }

            var last1 = ignored.Last();
            var last2 = pairs1.Last();

            Assert.IsTrue(last1.Position > last2.Key, "last1.Position > last2.Key");
        }

        // test that last ignored is 0107

        var binaryStreamRegion = ignored.Last();
        var binaryReader       = new BinaryReader(stream);
        binaryReader.BaseStream.Position = binaryStreamRegion.Position;
        var readNode            = DMDNode.ReadNode(null, binaryReader);
        Assert.IsTrue(readNode is DMDNode0107, "readNode is DMDNode0107");
        var binaryStreamRegions = stream.GetRegions(BinaryStreamRegionKind.Reading, BinaryStreamRegionType.Ignored).ToArray();
        CollectionAssert.AreEqual(regions, binaryStreamRegions);
