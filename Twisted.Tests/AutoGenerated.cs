...
public static class AutoGenerated
{
    public static void Test(TestContext context, string path)
    {
        ...
        const bool printNodes    = true; // print node families, ordinal sorted by content
        const bool printPolygons = true; // print polygon families, ordinal sorted by content
        ...
        if (printNodes)
            PrintNodes(context, treeNode, path);

        if (printPolygons)
            PrintPolygons(context, treeNode, path);
    }
    ...
    private static void PrintNodes(TestContext context, TreeNode treeNode, string path)
    {
        if (context == null)
            throw new ArgumentNullException(nameof(context));
        ...
        context.WriteLine("Nodes grouped by type and sorted by content:");
        context.WriteLine();

        var binaryObjects = treeNode.TraverseDfsPreOrder().Cast<IBinaryObject>().ToArray();

        var       length = binaryObjects.Max(s => s.Length) + 4;
        using var stream = File.Create($"{path}.nodes.bin");
        
        foreach (var grouping in binaryObjects.GroupBy(s => s.GetType()).OrderBy(s => s.Key.Name))
        {
            context.WriteLine($"{grouping.Key.Name}:");

            foreach (var binaryObject in grouping.OrderBy(s => s.GetObjectData(), ArrayComparer<byte>.Instance))
            {
                ...
                context.WriteLine($"{binaryObject.Position,6}: {string.Join(" ", data.Select(s => s.ToString("X2")))}");
            }
    ...
    private static void PrintPolygons(TestContext context, TreeNode treeNode, string path)
    {
        if (context == null)
            throw new ArgumentNullException(nameof(context));
        ...
        if (treeNode is DMDNode dmdNode)
        {
            var equalityComparer = new DelegateEqualityComparer<Polygon>((x, y) =>
            {
                if (x == null && y == null)
                    return true;
            ...
            var polygons1 = dmdNode.TraverseDfsPreOrder().OfType<DMDNode00FF>().SelectMany(s => s.Polygons).ToArray();
            var polygons2 = polygons1.Distinct(equalityComparer).ToArray();

            var       maxLength = polygons2.Max(s => s.Length) + 4;
            using var stream    = File.Create($"{path}.polygons.bin");

            foreach (var grouping in polygons2.ToLookup(s => s.GetType()))
            {
                context.WriteLine($"{grouping.Key.Name}:");

                foreach (var polygon in grouping.OrderBy(s => s.GetObjectData(), ArrayComparer<byte>.Instance))
                {
                    var data = polygon.GetObjectData();

                    stream.Write(data);

                    for (var i = 0; i < maxLength - data.Length; i++)
                    {
                        stream.WriteByte(0xCD);
                    }

                    context.WriteLine($"{polygon.Position,6}: {string.Join(" ", polygon.GetObjectData().Select(s => s.ToString("X2")))}");
                }
        ...
        {
            throw new NotSupportedException("Only DMD is supported.");
        }
    }
