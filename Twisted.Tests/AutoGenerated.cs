...
public static class AutoGenerated
{
    ...
    private static void PrintNodes(TestContext context, TreeNode treeNode)
    {
        if (context == null)
            throw new ArgumentNullException(nameof(context));
        ...
        foreach (var grouping in binaryObjects.GroupBy(s => s.GetType()))
        {
            ...
            {
                context.WriteLine($"{binaryObject.Position,6}: {string.Join(" ", binaryObject.GetObjectData().Select(s => s.ToString("X2")))}");
            }
