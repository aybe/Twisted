...
public sealed partial class AutoGenerated
{
    ...
    private void Test1(string path)
    {
        ...
        var baseOffset = reader.ReadInt32(Endianness.LE);

        var nodeReader = new DPCNodeReader(reader, baseOffset);

        // read table of contents

        stream.Position = nodeReader.ReadAddress();
        ...
        while (queue.Any())
        {
            ...
            switch (nodeType)
            {
                ...
                case 0x_0107_F802:
                    ...
                    break;
                case 0x_0107_0100:
                    node = new DPCNode01070100(nodeReader, out nodeChildren);
                    break;
                case 0x_0207_0000:
                case 0x_0206_0000:
                case 0x_0208_0000:
        ...
        while (stack.Any())
        {
            ...
            TestContext.WriteLine($"{new string('\t', node.Depth)}{node.GetType().Name.Replace(nameof(DPCNode), string.Empty)} @ {node.Position}");

            var children = node.Children.AsEnumerable().Reverse();
