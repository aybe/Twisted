...
public class AutoGenerated
{
    ...
    private static void PrintNodes(TestContext context, TreeNode treeNode)
    {
        ...
        if (treeNode is not DMDNode)
            throw new NotSupportedException("Only DMD is supported.");

        context.WriteLine("Nodes grouped by type and sorted by content:");
        context.WriteLine();

        var dictionary = TreeNodeUtility.GetDictionaryOfNodes(treeNode);

        PrintDictionaryOfBinaryObjects(context, dictionary);
    }
    ...
    private static void PrintPolygons(TestContext context, TreeNode treeNode)
    {
        ...
        context.WriteLine("Polygons grouped by type and sorted by content:");
        context.WriteLine();

        var dictionary = TreeNodeUtility.GetDictionaryOfPolygons(treeNode);

        PrintDictionaryOfBinaryObjects(context, dictionary);
    }

    private static void PrintDictionaryOfBinaryObjects(TestContext context, Dictionary<Type, List<IBinaryObject>> dictionary)
    {
        if (context == null)
            throw new ArgumentNullException(nameof(context));
        ...
        foreach (var (type, binaryObjects) in dictionary)
        {
